...
def get_models():
    return glob.glob(os.path.join(script_dir, "*.pth"));
...
def test_model(path, silent=False, adv=False, TOTAL_RUNS=10, outside_bar=None, ns=None):
    ...
    return {"correct": correct, "total_n": total_n, "props": props};
...
if __name__ == "__main__":
    ...
    res = test_model(mname, silent, adv, TOTAL_RUNS)
    print(f"Correct: {res["correct"]}/{res["total_n"]} ({round((res["correct"]/res["total_n"])*100, 2)}%)")
    
    if adv:
        print("Proportions: ")
        for pr in range(len(res["props"])):
            print(f"    {pr}: {round(res['props'][pr]/sum(res['props'])*100,2)}%");